...
class KVStoreDist : public KVStoreLocal {
 public:
  explicit KVStoreDist(bool use_device_comm)  //使用device
      : KVStoreLocal(use_device_comm), ps_worker_(nullptr), server_(nullptr) {
    if (IsWorkerNode()) {  //是worker节点  负责干活的
      ps_worker_ = new ps::KVWorker<real_t>(0); //real_t float
  ...
  void Barrier() override {
    ps::Postoffice::Get()->Barrier(ps::kWorkerGroup); //Barrier() 函数来实现结点间的同步
  }

  void SendCommandToServers(int cmd_id,   //发送命令到servers
